...
const Accesos = () => {
  ...
  const opcionesPermiso = permisos.map((p) => ({
    value: p.id.toString(),
    label: `${p.permiso} | ${p.Sucursale?.codigo_sucursal || "Sin sucursal"} | ${p.Sucursale?.nombre || "Sin nombre"}`,
  }));
  ...
  // Agrupar por usuario
  const accesosAgrupados = accesos.reduce((acc, acceso) => {
 //   const codigo = acceso.Usuario?.codigo || "SIN_CODIGO";
    const codigo = acceso.Usuario?.nombre + " " + acceso.Usuario?.apellido || "------";
    if (!acc[codigo]) acc[codigo] = [];
    acc[codigo].push(acceso);
    return acc;
  }, {});
  ...
  return (
    <div className="p-4">
      ...
      {/* Renderizar una tabla por cada usuario */}
      {Object.keys(accesosAgrupados).map((codigoUsuario) => (
        <div key={codigoUsuario} className="mb-8 border border-gray-400 rounded-lg overflow-hidden">
          ...
          <table className="min-w-full border border-gray-300 text-sm md:text-base">
            ...
            <tbody>
              {accesosAgrupados[codigoUsuario].map((acceso, index) => (
                <tr
                  key={acceso.id}
                  className={`${index % 2 === 0 ? "bg-white" : "bg-gray-200"} hover:bg-blue-200`}
                >
                  ...
                  <td className="px-4 py-2 border">
                    {acceso.Permiso?.Sucursale?.codigo_sucursal
                      ? `${acceso.Permiso?.Sucursale?.codigo_sucursal} | ${acceso.Permiso?.Sucursale?.nombre}`
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-2xl">
            <h3 className="text-xl font-bold mb-4">
              {editMode ? "Editar Acceso" : "Agregar Nuevo Acceso"}
            </h3>
