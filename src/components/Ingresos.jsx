...
// conexiones a la API
import { listIngresosLazy, AddIngresos } from "../api/ingresos";
import { listTipoDocumento002 } from "../api/hacienda"
import { listProductosLazy } from "../api/productos";
import { listProveedor } from "../api/proveedores";
...
const Ingresos = () => {
  ...
  const fetchData = async (p = 1) => {
    try {
      setEditMode(false);
      setLoading(true);
      const data = await listIngresosLazy(p, limit, buscar);
  ...
  const fetchProveedores = async () => {
    try {
      ...
      setProveedores(data.data.map(p => ({ value: p.id, label: p.name })));
    } catch (error) {
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold">Gestión de Ingresos</h2>
        <button
          onClick={() => {
            fetchProveedores();
            setShowModal(true);
